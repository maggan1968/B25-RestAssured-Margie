...
public class HRPojoGetRequestTest extends HrTestBase {
    ...
    //try to store regions inside thr list<region>
    @DisplayName("GET all the regions inside the List<region>")
    @Test

    public void test3() {
        JsonPath jsonPath = get("/regions").then().statusCode(200).extract().jsonPath();

        Region.Regions regions = jsonPath.getObject("", Region.Regions.class);
        System.out.println("regions = " + regions.getItems().get(0).getRegion_name());

        for (Region item : regions.getItems()) {
            System.out.println(item.getRegion_name() + " " + item.getRegion_id());
        }
